...
private:
  ...
  // Change from static array to pointers:
  static inline uint8_t* _fb[_tft_bufcount] = {nullptr, nullptr};
...
public:
  ...
  explicit TPC_LCD(View &root, log_type log = nullptr)
    ...
    for (size_t i = 0; i < _tft_bufcount; ++i) {
      _fb[i] = (uint8_t*)heap_caps_malloc(_tft_bufsize, MALLOC_CAP_DMA | MALLOC_CAP_8BIT);
      if (!_fb[i]) {
        abort();
      }
    }
  ...
  bool init() override {
    ...
    // Use the dynamically allocated buffers:
